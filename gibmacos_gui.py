...
class GibMacOSBackend:
    def __init__(self, update_callback=None, progress_callback=None, cancel_event=None):
        ...
        self.settings_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "Scripts", "settings.json"
        )
        self.settings = {}
        ...
        self.prod_cache_path = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "Scripts", "prod_cache.plist"
        )
        self.prod_cache = {}
    ...
    def get_catalog_data(self):
        ...
        local_catalog = os.path.join(
            os.path.dirname(os.path.realpath(__file__)), "Scripts", "sucatalog.plist"
        )
    ...
    def get_build_version(self, dist_dict):
        ...
        try:
            ...
            pass
        try:
            device_ids = re.search(
                r"var supportedDeviceIDs\s*=\s*\[([^]]+)\];", dist_file
            )[1]
            device_ids = list(
                set(i.lower() for i in re.findall(r"'([^',]+)'", device_ids))
            )
        except:
            device_ids = []
    ...
    def download_prod(self, prod, download_dir, dmg=False):
            ...
            try:
                ...
                if result is None:
                    if self.cancel_event and self.cancel_event.is_set():
                        ...
                        raise Exception(
                            "Download failed or was interrupted (no specific error)."
                        )
        ...
        if failed_downloads:
            raise ProgramError(
                f"{len(failed_downloads)} files failed to download: {', '.join(failed_downloads)}",
...
class GibMacOSGUI(tk.Tk):
        ...
        if total > 0:
            ...
            if elapsed_time > 0 and current > 0:
                ...
                self.progress_bar_label.config(
                    text=f"{percent:.2f}% ({self.backend.d.get_size(current)} / {self.backend.d.get_size(total)}) - {speed_str} - ETA {eta_str}"
                )
            else:
                self.progress_bar_label.config(
                    text=f"{percent:.2f}% ({self.backend.d.get_size(current)} / {self.backend.d.get_size(total)})"
                )
        else:
    ...
    def _create_widgets(self):
        ...
        self.products_frame = ttk.LabelFrame(
            self, text="Available macOS Products", padding="10"
        )
        ...
        self.bottom_panel_frame.pack(
            side=tk.BOTTOM, fill=tk.BOTH, expand=True, padx=10, pady=5
        )
    ...
    def _open_download_dir(self):
        try:
            ...
            self._queue_error_dialog(
                "Error Opening Directory", f"Could not open download directory: {e}"
            )
    ...
    def _download_selected(self):
        ...
        selected_prod = next(
            (p for p in self.gui_products_data if p["product"] == selected_item_id),
            None,
        )
...
"""
        ...
        text = scrolledtext.ScrolledText(
            text_frame, wrap=tk.WORD, font=("Helvetica", 11), padx=10, pady=10
        )
    ...
    def _show_about(self):
        ...
        # Header
        ttk.Label(main_frame, text="gibMacOS GUI", font=("Helvetica", 16, "bold")).pack(
            pady=(0, 10)
        )

        ttk.Label(
            main_frame, text="Version 2.0 (GUI Edition)", font=("Helvetica", 12)
        ).pack(pady=(0, 20))
        ...
        original_link.bind(
            "<Button-1>",
            lambda e: webbrowser.open("https://github.com/corpnewt/gibMacOS"),
        )
...
if __name__ == "__main__":
    app = GibMacOSGUI()
    print("✅ The GUI version running successfully.")
    app.mainloop()
